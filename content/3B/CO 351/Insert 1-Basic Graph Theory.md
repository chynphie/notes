...
> A matrix with $|V|$ rows and $|E|$ columns with entires $\in \{ 0,-1,1 \}$ s.t.
> 1. Rows correspond to nodes of G
> 2. Columns correspond to the arcs of G
> 

> [!definition] (Di)Path
> is a digraph consisting of $k$ distink nodes $v_1,..,v_k$ and $k-1$ distinct arcs, where ith arc is ($v_{i}v_{i+1}$ ) either $v_iv_{i+1}\:or\:v_{i+1}v_i$
 
> [!definition] (Di)cycle 
> similar idea to (Di)path

> [!definition] Connected Digraph
> If for every pair of nodes $i$ and $j$, digraph contains a path between $i$ and $j$
